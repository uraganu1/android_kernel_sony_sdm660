	...
	case FE_SET_TONE:
		if (fe->ops.set_tone) {
			err = fe->ops.set_tone(fe,
					       (enum fe_sec_tone_mode)(*(int *)parg));
			fepriv->tone = (enum fe_sec_tone_mode)(*(int *)parg);
			fepriv->state = FESTATE_DISEQC;
			fepriv->status = 0;
		}
		break;
	...
	case FE_SET_VOLTAGE:
		if (fe->ops.set_voltage) {
			err = fe->ops.set_voltage(fe,
						  (enum fe_sec_voltage)(*(int *)parg));
			fepriv->voltage = (enum fe_sec_voltage)(*(int *)parg);
			fepriv->state = FESTATE_DISEQC;
			fepriv->status = 0;
		}
		break;
